...
public class CoursService {
    ...
    public boolean add(Cours c) {
        String req = "insert into cours(titre,etat,contenu,id_cat) values (?,?,?,?)";
        try {
    ...
    public List<Cours> readAll() {
        String req = "SELECT c.id, c.titre, c.etat, c.contenu, c.id_cat, cat.* FROM cours c JOIN categorie cat ON c.id_cat = cat.id";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                list.add(new Cours(id,titre,etat, contenu, categorie));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
